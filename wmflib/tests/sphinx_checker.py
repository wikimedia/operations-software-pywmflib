...
def main(base_path):
    """Perform the check."""
    wmflib_modules = {
        name for _, name, ispkg in pkgutil.iter_modules(wmflib.__path__) if not ispkg and name not in EXCLUDED_NAMES
    }
    ...
    if doc_api_modules - wmflib_modules:
        print(
            f"Documented modules in {DOC_API_INDEX_PATH} that are missing in wmflib: {doc_api_modules - wmflib_modules}"
        )
        ret += 1

    doc_api_files = [f"wmflib.{name}.rst" for name in doc_api_modules]
    missing_doc_api_files = [file for file in doc_api_files if not (DOC_API_BASE_PATH / file).is_file]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Check that all wmflib modules are documented")
